...
const laboratoryContributorTotalPriceUpdate = (nbDaysField, nbHoursField, dailyPriceField, totalPriceField) => {
    const result = calculateContributorPrice(nbDaysField, nbHoursField, !dailyPriceField.val() ? 0 : dailyPriceField.val())
    totalPriceField.val(result)
}
 ...
 */
const hideOrShowLabelBlock = (labelBlock, nbLinesDuplicated) => {
    if (nbLinesDuplicated != 0) labelBlock.show()
    else labelBlock.hide()
}
