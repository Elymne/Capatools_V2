...
const equipmentsData = JSON.parse(
    document.getElementById("equipments-data-target").textContent
);
...
$(() => {
     ...
     */
    equipmentsLocalStateList = getEquipementsListFromSelectedLaboratory(
        equipmentsData,
        laboratoriesData
    );
    ...
    addedEquipementsOnInit.forEach((elem, i) => {
         ...
         */
        $(
            `#projectcreateequipmentrepaymentform-${i}-equipmentselected`
        ).empty();
        $(`#projectcreateequipmentrepaymentform-${i}-equipmentselected`).append(
            getEquipementsOptionsListFromSelectedLaboratory(
                equipmentsData,
                laboratoriesData
            )
        );
        ...
        $(`#projectcreateequipmentrepaymentform-${i}-timeriskstringify`).val(
        ...
        $(`#projectcreateequipmentrepaymentform-${i}-equipmentselected`).change(
            () => {
                const result = calculateEquipmentPrice(
                    ...
                    getValueFromEquipmentSelectedByIndex(
                        equipmentsLocalStateList,
                        i
                    )
                );
                $(`#projectcreateequipmentrepaymentform-${i}-price`).val(
                    result
                );
            }
        );
         ...
         * Permet de reclaculer le coût d'utilisation d'un matériel.
         */
        $(`#projectcreateequipmentrepaymentform-${i}-nb_days`).on(
            "input",
            () => {
                const result = calculateEquipmentPrice(
                    !$(
         ...
         */
        $(`#projectcreateequipmentrepaymentform-${i}-nb_hours`).on(
            "input",
            () => {
                const result = calculateEquipmentPrice(
                    !$(
    ...
    addedContributorsOnInit.forEach((elem, i) => {
        ...
        $(`#projectcreatelaboratorycontributorform-${i}-timeriskstringify`).val(
            stringifyRiskTime(riskObject.riskDay, riskObject.riskHour)
        );
         ...
         */
        $(`#projectcreatelaboratorycontributorform-${i}-nb_days`).on(
            "input",
            () => {
                const result = calculateContributorPrice(
                    !$(
                ...
                $(`#projectcreatelaboratorycontributorform-${i}-price`).val(
                    result
                );
         ...
         * Permet de recalculer le coût d'une intervention.
         */
        $(`#projectcreatelaboratorycontributorform-${i}-nb_hours`).on(
            "input",
            () => {
                const result = calculateContributorPrice(
                    !$(
...
$(() => {
    ...
    $(".dynamicform_wrapper_equipment").on("afterInsert", (e, item) => {
         ...
         */
        $(
            `#projectcreateequipmentrepaymentform-${nbEquipmentLineDuplicated}-equipmentselected`
        ).empty();
        $(
            `#projectcreateequipmentrepaymentform-${nbEquipmentLineDuplicated}-equipmentselected`
        ).append(
        ...
        $(
            `#projectcreateequipmentrepaymentform-${nbEquipmentLineDuplicated}-timeriskstringify`
        ).val(stringifyRiskTime());

        // Par défaut, le coût généré est de 0.
        $(
            `#projectcreateequipmentrepaymentform-${nbEquipmentLineDuplicated}-price`
        ).val(0);
...
$(() => {
    ...
    $(".dynamicform_wrapper_contributor").on("afterInsert", (e, item) => {
        ...
        // Par défaut, le temps généré par le risque est à 0.
        $(
            `#projectcreatelaboratorycontributorform-${nbContributorLineDuplicated}-timeriskstringify`
        ).val(stringifyRiskTime());

        // Par défaut, le coût généré est de 0.
        $(
            `#projectcreatelaboratorycontributorform-${nbContributorLineDuplicated}-price`
        ).val(0);
 ...
 */
const getValueFromLaboratorySelected = (laboratoriesData) =>
    laboratoriesData[
        $(
            "#projectcreaterepaymentform-laboratoryselected option:selected"
        ).val()
    ];
...
const getValueFromEquipmentSelectedByIndex = (equipmentsData, index = 0) =>
    equipmentsData[
        $(
            `#projectcreateequipmentrepaymentform-${index}-equipmentselected option:selected`
        ).val()
    ];
 ...
 * @returns Un prix.
 */
const calculateEquipmentPrice = (nbDay, nbHour, equipment) =>
    equipment.price_day * nbDay + equipment.price_hour * nbHour;
 ...
 * @returns Un prix.
 */
const calculateContributorPrice = (nbDay, nbHour, laboratory) =>
    laboratory.price_contributor_day * nbDay +
    laboratory.price_contributor_hour * nbHour;
...
const stringifyRiskTime = (riskDay = 0, riskHour = 0) =>
    `${parseFloat(riskDay).toFixed(0)}j ${parseFloat(riskHour).toFixed(0)}h`;
 ...
 */
const getEquipementsOptionsListFromSelectedLaboratory = (
    equipmentsData,
    laboratoriesData
) => {
    dataToRefresh = equipmentsData.filter(
        (element) =>
            element.laboratory_id ==
            getValueFromLaboratorySelected(laboratoriesData).id
    );
 ...
 * @param {*} laboratoriesData
 */
const getEquipementsListFromSelectedLaboratory = (
    equipmentsData,
    laboratoriesData
) =>
