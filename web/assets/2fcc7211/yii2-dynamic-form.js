...
(function ($) {
    ...
    $.fn.yiiDynamicForm = function (method) {
        if (methods[method]) {
            ...
            $.error(
                "Method " + method + " does not exist on jQuery.yiiDynamicForm"
            );
            return false;
        }
    };
    ...
    var _parseTemplate = function (widgetOptions) {
        ...
        $template.find("div[data-dynamicform]").each(function () {
            ...
            if ($(widgetOptions.widgetItem).length > 1) {
                var item = $(this).find(widgetOptions.widgetItem).first()[0]
                    .outerHTML;
        ...
        $template
            ...
            .each(function () {
                ...
                var $inputHidden = $template
                    .find('input[type="hidden"][name="' + inputName + '"]')
                    .first();
                if ($inputHidden) {
                    $(this).val(1);
                    $inputHidden.val(0);
    ...
    var _getWidgetOptionsRoot = function (widgetOptions) {
        return eval(
            $(widgetOptions.widgetBody)
                .parents("div[data-dynamicform]")
                .last()
                .attr("data-dynamicform")
        );
    };
    ...
    var _count = function ($elem, widgetOptions) {
        return $elem
            .closest("." + widgetOptions.widgetContainer)
            .find(widgetOptions.widgetItem).length;
    };
    ...
    var _removeValidations = function ($elem, widgetOptions, count) {
        if (count > 1) {
            $elem.find("div[data-dynamicform]").each(function () {
                var currentWidgetOptions = eval(
                    $(this).attr("data-dynamicform")
                );
                var level = _getLevel($(this));
                var identifiers = _createIdentifiers(level);
                var numItems = $(this).find(currentWidgetOptions.widgetItem)
                    .length;
                ...
                for (var i = 1; i <= numItems - 1; i++) {
                    ...
                    currentWidgetOptions.fields.forEach(function (input) {
                        ...
                        if (
                            $("#" + currentWidgetOptions.formId).yiiActiveForm(
                                "find",
                                id
                            ) !== "undefined"
                        ) {
            ...
            widgetOptions.fields.forEach(function (input) {
                ...
                if (
                    $("#" + widgetOptions.formId).yiiActiveForm("find", id) !==
                    "undefined"
                ) {
    ...
    var _deleteItem = function (widgetOptions, e, $elem) {
        ...
        if (count > widgetOptions.min) {
            ...
            // trigger a custom event for hooking
            var eventResult = $(
                "." + widgetOptions.widgetContainer
            ).triggerHandler(events.beforeDelete, $todelete);
            if (eventResult !== false) {
                ...
                _restoreSpecialJs(widgetOptions);
                _fixFormValidaton(widgetOptions);
                $("." + widgetOptions.widgetContainer).triggerHandler(
                    events.afterDelete
                );
            }
        }
    ...
    var _updateAttrID = function ($elem, index) {
            ...
            if (matches && matches.length === 4) {
                ...
                if (identifiers.length > 1) {
                    ...
                    for (var i = identifiers.length - 1; i >= 1; i--) {
                        ...
                        identifiers[i] = $elem
                            .closest(widgetsOptions[i - 1].widgetItem)
                            .index();
                    }
                }
        ...
        if (id !== newID) {
            $elem
                .closest(widgetOptions.widgetItem)
                .find(".field-" + id)
                .each(function () {
                    $(this)
                        .removeClass("field-" + id)
                        .addClass("field-" + newID);
                });
        ...
        $(widgetOptionsRoot.widgetBody)
            ...
            .each(function () {
                ...
                if (id !== undefined && name !== undefined) {
                    currentWidgetOptions = eval(
                        $(this)
                            .closest("div[data-dynamicform]")
                            .attr("data-dynamicform")
                    );
                    var matches = id.match(regexID);

                    if (matches && matches.length === 4) {
                        matches[2] = matches[2].substring(
                            1,
                            matches[2].length - 1
                        );
                        var level = _getLevel($(this));
                        ...
                        var baseID =
                            ...
                            matches[3];
                        var attribute = $(
                            "#" + currentWidgetOptions.formId
                        ).yiiActiveForm("find", baseID);
    ...
    var _restoreSpecialJs = function (widgetOptions) {
        ...
        var $hasTimepicker = $(widgetOptionsRoot.widgetItem).find(
        ...
        if ($hasTimepicker.length > 0) {
            $hasTimepicker.each(function () {
                ...
                $(this).unbind();
                $(this).timepicker(
                    eval($(this).attr("data-krajee-timepicker"))
                );
            });
        }

        // "kartik-v/yii2-money"
        var $hasMaskmoney = $(widgetOptionsRoot.widgetItem).find(
        ...
        if ($hasMaskmoney.length > 0) {
            $hasMaskmoney.each(function () {
                ...
                $(displayID).maskMoney(
                    eval($(this).attr("data-krajee-maskMoney"))
                );
        ...
        var $hasFileinput = $(widgetOptionsRoot.widgetItem).find(
            "[data-krajee-fileinput]"
        );
        ...
        var $hasTouchSpin = $(widgetOptionsRoot.widgetItem).find(
            "[data-krajee-TouchSpin]"
        );
        ...
        var $hasSpectrum = $(widgetOptionsRoot.widgetItem).find(
            "[data-krajee-spectrum]"
        );
        ...
        var $hasDepdrop = $(widgetOptionsRoot.widgetItem).find(
            "[data-krajee-depdrop]"
        );
        if ($hasDepdrop.length > 0) {
            $hasDepdrop.each(function () {
                ...
                if (matchID && matchID.length === 4) {
                    for (
                        index = 0;
                        index < configDepdrop.depends.length;
                        ++index
                    ) {
                        var match = configDepdrop.depends[index].match(regex);
                        if (match && match.length === 4) {
                            configDepdrop.depends[index] =
                                match[1] + matchID[2] + match[3];
                        }
                    }
                }
        ...
        var $hasSelect2 = $(widgetOptionsRoot.widgetItem).find(
            "[data-krajee-select2]"
        );
        if ($hasSelect2.length > 0) {
            $hasSelect2.each(function () {
                ...
                var s2LoadingFunc =
                    typeof initSelect2Loading != "undefined"
                        ? initSelect2Loading
                        : initS2Loading;
                var s2OpenFunc =
                    typeof initSelect2DropStyle != "undefined"
                        ? initSelect2Loading
                        : initS2Loading;
                $.when($("#" + id).select2(configSelect2)).done(
                    s2LoadingFunc(id, ".select2-container--krajee")
                );
                ...
                if (configDepdrop) {
                    var loadingText = configDepdrop.loadingText
                        ? configDepdrop.loadingText
                        : "Loading ...";
            ...
            $hasSelect2.each(function () {
                ...
                if ($(this).attr("data-krajee-depdrop")) {
                    $(this).on("depdrop.beforeChange", function (e, i, v) {
                        var configDepdrop = eval(
                            $(this).attr("data-krajee-depdrop")
                        );
                        var loadingText = configDepdrop.loadingText
                            ? configDepdrop.loadingText
                            : "Loading ...";
